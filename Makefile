...
build-darwin-arm64:
	...
	cd $(BACKEND_DIR) && GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -ldflags="-s -w" -o ../$(DARWIN_ARM64_DIR)/cloudviz/$(BINARY_NAME) main.go azure.go db.go types.go dependencies.go webhooks.go
...
build-darwin-amd64:
	...
	cd $(BACKEND_DIR) && GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-s -w" -o ../$(DARWIN_AMD64_DIR)/cloudviz/$(BINARY_NAME) main.go azure.go db.go types.go dependencies.go webhooks.go
...
build-linux:
	...
	cd $(BACKEND_DIR) && GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-s -w" -o ../$(LINUX_AMD64_DIR)/cloudviz/$(BINARY_NAME) main.go azure.go db.go types.go dependencies.go webhooks.go
